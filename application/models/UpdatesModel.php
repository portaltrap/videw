...
class UpdatesModel extends CI_Model {
    ...
    public function fetch_info() {
		return json_decode(getRemoteContents(DOWNLOADS_URL . PRODUCT_ID . '/' . PRODUCT_ID . '.json'), true);;
    }
	...
	public function paste_config_details($base_url,$encryption_key) {
		$sampleFile = file_get_contents(APPPATH."config/config.php");
		$sampleFile = str_replace("{{base_url}}",$base_url,$sampleFile);
		$sampleFile = str_replace("{{encryption_key}}",$encryption_key,$sampleFile);
