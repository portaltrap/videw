typeof navigator === "object" && (function (global, factory) {
  ...
  var defaults = {
    ...
    loadSprite: true,
    iconPrefix: 'plyr',
    iconUrl: '../application/views/themes/neo_red/assets/player/plyr.svg',
  ...
  /**
   * Notes:
   * - Thumbs are set via JS settings, not HTML5 'track' property. Using the track property would be a bit gross to me, because it doesn't support custom 'kinds'. kind=metadata might be used for something else, and we want to allow multiple thumbnails tracks. Tracks must have a unique combination of 'kind' and 'label'. We would have to do something like kind=metadata,label=thumbnails1 / kind=metadata,label=thumbnails2. Square peg, round hole
    ...
    _createClass(PreviewThumbnails, [{
      ...
      value: function load() {
        ...
        this.getThumbnailsDefs().then(function () {
          ...
          _this.elements(); // Turn off the regular seek tooltip


          _this.player.config.tooltips.seek = false; // Check to see if thumb container size was specified manually in CSS
      ...
      value: function listeners() {
        ...
        on.call(this.player, this.player.elements.progress, 'mousedown touchstart', function () {
      ...
      value: function showImageAtCurrentTime() {
        ...
        this.setThumbContainerSizeAndPos(); // Check when we last loaded an image - don't show more than one new one every 500ms

        if (this.lastMousemoveEventTime < Date.now() - 150) {
          this.lastMousemoveEventTime = Date.now(); // Find the first thumbnail that's after `time`

          var thumbNum = this.thumbnailsDefs[0].frames.findIndex(function (frame) {
            return _this5.seekTime + 1 >= frame.startTime && _this5.seekTime <= frame.endTime;
          ...
          if (!this.mousemoveEventTimeout) {
            this.mousemoveEventTimeout = setTimeout(function () {
              // Don't follow through after the timeout if it's since been hidden
              if (_this5.player.elements.display.previewThumbnailContainer.style.opacity === 1) {
                _this5.showImageAtCurrentTime();

                _this5.mousemoveEventTimeout = null;
              }
            }, 200);
          }
        }
      ...
      value: function showImage() {
        ...
        var localImageTryShowCounter = this.imageTryShowCounter;
        ...
        this.player.debug.log("Preview thumbnails: showing thumbnum: ".concat(thumbNum, ": ").concat(JSON.stringify(this.thumbnailsDefs[qualityIndex].frames[thumbNum])));
		...
		previewImage.setAttribute('style', 'background-image: url('+thumbURL+'); background-position: -'+xpos+'px -'+ypos+'px;');
		previewImage.setAttribute('width', wwidth);
		previewImage.setAttribute('height', hheight);
        ...
        previewImage.onload = function () {
          ...
          if (localImageTryShowCounter >= _this6.imageShowCounter) {
            _this6.imageShowCounter = localImageTryShowCounter;

            _this6.currentContainer.appendChild(previewImage); // Now that this one is showing, start pre-loading a batch of nearby images. But only if this isn't a revisit
            // this.preloadNearbyImages(thumbNum);


            _this6.thumbnailsDefs[qualityIndex].frames[thumbNum].loaded = true;

            _this6.removeOldImages(); // Look for a higher quality version of the same frame


            if (qualityIndex < _this6.thumbnailsDefs.length - 1) {
              // Only use the higher quality version if it's going to look any better - if the current thumb is of a lower pixel density than the thumbnail container
              var previewContainerHeight = _this6.player.elements.display.previewThumbnailContainer.clientHeight;
              if (_this6.mouseDown) previewContainerHeight = _this6.player.elements.display.previewScrubbingContainer.clientHeight; // Adjust for HiDPI screen
              ...
              if (previewImage.naturalHeight < previewContainerHeight) {
                ...
                setTimeout(function () {
                  ...
                  if (_this6.showingThumb === thumbNum) {
                    _this6.showImage(qualityIndex + 1);
                  }
                }, 150);
              }
            }
      ...
      key: "preloadNearbyImages",
      value: function preloadNearbyImages(thumbNum) {
        var amountToPreload = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 30;

        var actualShowingThumb = _toConsumableArray(this.currentContainer.children).reverse()[0].getAttribute('data-thumbnum');

        if (actualShowingThumb && Number(actualShowingThumb) === this.showingThumb) {
          var startNum = thumbNum - amountToPreload / 2;
          var endNum = thumbNum + amountToPreload / 2;
          ...
          for (var i = startNum; i <= endNum; i++) {
            if (!this.thumbnailsDefs[0].frames[i].loaded) {
              ...
              var thumbFilename = this.thumbnailsDefs[0].frames[i].text;
              var urlPrefix = this.thumbnailsDefs[0].urlPrefix;
              var thumbURL = urlPrefix + thumbFilename; // We're building and adding a new image. In other implementations of similar functionality (Youtube), background image is instead used. But this causes issues with larger images in Firefox and Safari - switching between background images causes a flicker. Putting a new image over the top does not

              var previewImage = new Image();
              previewImage.src = thumbURL; // Set loaded attribute. This will prevent us from wasting CPU constantly trying to preload images that we already have loaded
      ...
      key: "removeOldImages",
      value: function removeOldImages() {
        // Get a list of all images, and reverse it - so that we can start from the end and delete all except for the most recent
        var allImages = _toConsumableArray(this.currentContainer.children).reverse(); // Start at the third image image - so we leave the last two images. Leaving only one might result in flickering if the newest one hasn't finished rendering yet


        for (var i = 2; i < allImages.length; i++) {
      ...
      value: function setThumbContainerSizeAndPos() {
        ...
        if (!this.sizeSpecifiedInCSS) {
          var videoAspectRatio = this.player.media.videoWidth / this.player.media.videoHeight;
          var thumbHeight = this.player.elements.container.clientHeight / 4;
          var thumbWidth = thumbHeight * videoAspectRatio;
          this.player.elements.display.previewThumbnailContainer.style.height = "".concat(thumbHeight, "px");
      ...
      value: function parseVtt(vttDataString) {
        ...
        try {
          for (var _iterator2 = frames[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
            ...
            try {
              for (var _iterator3 = frame.split(/\r\n|\n|\r/)[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
